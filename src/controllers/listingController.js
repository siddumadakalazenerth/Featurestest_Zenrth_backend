...
const { createToolJob, createCustomEditJob, publicToolJob, GEMINI_SUPPORTED_TOOLS } = require('../services/toolOrchestrator');
...
async function reviewToolJob(req, res, next) {
  try {
    ...
    if (job.resultType === 'image') {
      ...
      if (!version || !photo || !version.diskPath) {
        return res.status(409).json({ error: 'The generated image preview is not available locally.' });
      }
