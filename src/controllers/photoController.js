...
const { createCustomEditJob, createFurnishingRenderJob, publicToolJob } = require('../services/toolOrchestrator');
...
async function uploadPhotos(req, res, next) {
  try {
    ...
    enqueuePhotos(createdPhotos.map((photo) => photo._id));
...
async function restoreVersion(req, res, next) {
  try {
    ...
    if (!version || !version.diskPath) return res.status(404).json({ error: 'Version not found' });
...
async function verifyCustomFurnishing(req, res, next) {
  try {
    ...
    if (result.fits && Array.isArray(result.pieces) && result.pieces.length > 0) {
      photo.furnishingSuggestion = {
        roomType: photo.furnishingSuggestion?.roomType || photo.analysis?.roomType || null,
        estimatedDimensions: photo.furnishingSuggestion?.estimatedDimensions || {},
        style: photo.furnishingSuggestion?.style || '',
